<x-layout title="Edit Bill">
    <form action="{{ route('bills.update', $bill) }}" method="POST" class="bg-white rounded-xl border border-slate-200 p-6 max-w-xl space-y-4">
        @csrf @method('PUT')
        <div>
        ...
        <div>
            ...
            <textarea name="notes" rows="3"
                      class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $bill->notes) }}</textarea>
        </div>
