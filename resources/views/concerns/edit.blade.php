<x-layout title="Edit Concern">
    <form action="{{ route('concerns.update', $concern) }}" method="POST" class="bg-white rounded-xl border border-[#e2e8f0] p-6 max-w-xl space-y-4">
        @csrf @method('PUT')
        <div>
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                      class="w-full rounded-lg border border-[#cbd5e1] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#6366f1]">{{ old('description', $concern->description) }}</textarea>
        </div>
        <div>
            <label class="block text-sm font-medium text-[#1e293b] mb-1">Resolution <span class="text-[#94a3b8] font-normal">(visible to the tenant)</span></label>
            <textarea name="resolution" rows="4" placeholder="e.g. Plumber visited on Sep 22 and replaced the washer. Issue fixed."
                      class="w-full rounded-lg border border-[#cbd5e1] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#6366f1]">{{ old('resolution', $concern->resolution) }}</textarea>
        </div>
        ...
        @if ($concern->status === 'resolved' || old('status') === 'resolved')
            <div>
                <label class="block text-sm font-medium text-[#1e293b] mb-1">Resolved date</label>
