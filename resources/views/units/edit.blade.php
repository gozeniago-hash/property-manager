<x-layout title="Edit Unit">
    <form action="{{ route('units.update', $unit) }}" method="POST" class="bg-white rounded-xl border border-[#e2e8f0] p-6 max-w-xl space-y-4">
        @csrf @method('PUT')
        <div>
        ...
        <div>
            ...
            <textarea name="notes" rows="3"
                      class="w-full rounded-lg border border-[#cbd5e1] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#6366f1]">{{ old('notes', $unit->notes) }}</textarea>
        </div>
